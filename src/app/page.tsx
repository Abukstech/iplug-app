...
export default function Home() {
  const trendingPhones = [
    { name: "iPhone Smart 13", price: "$ 999", image: phone1},
    { name: "iPhone Smart 12", price: "$ 799", image: phone2 },
    { name: "iPhone Smart 11", price: "$ 699", image: phone3 },
    { name: "iPhone Smart 10", price: "$ 599", image: phone4 }
  ];
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
            {trendingPhones.map((phone, index) => (
              <div key={index} className="bg-white p-4 rounded-lg shadow-md overflow-hidden">
                <div className=" bg-[#ECF4FF] flex items-center justify-center">
                  <Image src={phone.image} alt={phone.name} />
                </div>
                <div className="p-4">
                  <div className="flex flex-row gap-5 items-center ">
                  <h3 className="font-semibold">{phone.name}</h3>
                  <p className="text-blue-600 font-semibold mt-2">{phone.price}</p>
                  </div>
               

                  <button className="mt-2 text-sm text-blue-600">See more</button>
                </div>
              </div>
            ))}
          </div>
